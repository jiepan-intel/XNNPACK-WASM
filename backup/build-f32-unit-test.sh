...
#for testcase in qs8_qc8w_gemm_minmax_fp32_test
#for testcase in f32_gemm_test f32_gemm_relu_test f32_gemm_minmax_test

for testcase in f32_qs8_vcvt_test f32_qu8_vcvt_test f32_vhswish_test
do
${BAZEL} build  --action_env=HTTP_PROXY=TTP_PROXY     -c opt  --cpu=wasm  --features=wasm_simd  --copt="-Wno-error" --crosstool_top=@emsdk//emscripten_toolchain:everything --spawn_strategy=local --host_crosstool_top=@bazel_tools//tools/cpp:toolchain ${testcase} 2>&1 |tee log/${testcase}.log
done
